...
void FaultViewer(void)
{
	unsigned int * SP = CurrentStackPointer;
	UARTprintf("HARDFAULT\n");
...
void Ticker(void)
{
	...
	if(Mode==1)ContextSwitch();//go to scheduler if in task else return to scheduler
}
unsigned int ReturnTicker(void)
{
	return Ticks;
}
